...
func _ready():
	get_node("/root/Main/machines/bottom row/fix6/").visible = false
	$"machine status6".texture = null
	$"inventory6/inv num6".text = "0"
...
func workOnJob(delta):
	...
	if broken:
		...
		$"machine status6".texture = load("res://assets/machine status icons/machine_building.png")
		
	if isJobFinished():
		item_progress = 0
		itemNum += 1
		# update inventory stats
		$"inventory6/inv num6".text = str(itemNum)
		global.NUM_OF_CROSSBOWS = itemNum
		...
		if elasped_time >= 1:
			...
			elasped_time = 0
	$ProgressBar6.value = item_progress
	...
	if event is InputEventMouseButton and event.button_index == BUTTON_LEFT and get_node("/root/Main/machines/").BUILD_POP_UP_OPEN == false and get_node("/root/Main/machines/").UPGRADE_POP_UP_OPEN == false:
		if get_rect().has_point(get_local_mouse_position()):
			if event.pressed:
				...
				get_node("/root/Main/build pop up/pop up content/item name/").set_text("Wooden Crossbow")
				get_node("/root/Main/build pop up/pop up content/item sprite/").texture = load("res://assets/fantasy icons - cropped/tile100.png")
